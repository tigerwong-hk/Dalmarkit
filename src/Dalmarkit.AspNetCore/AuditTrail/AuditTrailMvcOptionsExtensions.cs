...
public static class AuditTrailMvcOptionsExtensions
{
    ...
    /// <param name="excludedControllerActions">controller actions to exclude from audit trail</param>
    public static MvcOptions AddGlobalAuditFilter(this MvcOptions mvcOptions, IReadOnlySet<(string ControllerName, string ActionName)> excludedControllerActions)
    {
        mvcOptions.AddAuditFilter(config => config
            .LogActionIf(x => !excludedControllerActions.Contains((x.ControllerName, x.ActionName)))
